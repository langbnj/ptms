...
foreach $mode (@modes)
{
    ...
    $query = Query("SELECT COUNT(DISTINCT ensp), COUNT(DISTINCT ensp, site), COUNT(*), MIN(rate), AVG(rate), MAX(rate) FROM evorate_lichtarge_$mode");
    ($ensps_evorate, $residues_evorate, $rows_evorate, $minrate, $avgrate, $maxrate) = FetchOne($query);
    state("   >> evorate_lichtarge_$mode >> ".commify($ensps_evorate)." ENSPs >> ".commify($residues_evorate)." residues >> MIN $minrate AVG $avgrate MAX $maxrate", 1);

    warn("Warning: 'evorate_lichtarge_$mode' should have ".commify($residues_comparafasta)." residues according to $comparafasta, but it has ".commify($residues_evorate)." residues") if ($residues_evorate != $residues_comparafasta);
    warn("Warning: 'evorate_lichtarge_$mode' has ".commify($residues_evorate)." residues, but ".commify($rows_evorate)." total rows") if ($residues_evorate != $rows_evorate);
    ...
    if ($mode eq 'para')
    {
        ...
        warn("Warning: 'evorate_lichtarge_$mode' is incomplete (".commify($ensps_evorate)." instead of the expected ".commify($ensps_comparafasta)." from $comparafasta) (".($ensps_comparafasta - $ensps_evorate)." missing, which is probably due to X-containing human ENSP sequences (there are $x_containing_ensps of them) for comparafasta (para))") if ($ensps_comparafasta ne $ensps_evorate);
    }
    else
    {
        warn("Warning: 'evorate_lichtarge_$mode' is incomplete (".commify($ensps_evorate)." instead of the expected ".commify($ensps_comparafasta)." from $comparafasta) (".($ensps_comparafasta - $ensps_evorate)." missing)") if ($ensps_comparafasta ne $ensps_evorate);
    }
}
...
foreach $mode (@modes)
{
    Optimize("evorate_lichtarge_$mode");
}
