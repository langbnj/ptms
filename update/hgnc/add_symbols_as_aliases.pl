...
startme("Getting all symbols from table 'hgnc' and inserting them into table 'hgnc_aliases' as pseudo-aliases (for easy lookup of e.g. HGNC ID numbers or ENSGs via the 'alias' column)", 0, Numrows($mainquery));
starttime();
while (($hgncid, $symbol, $ensg, $accs) = Fetch($mainquery))
{
	...
	else
	{
		...
		foreach $acc (nsort(split(/\|/, $accs)))
		{
			...
			else
			{
				...
				if (Numrows($query) == 0)
				{
					addme("uniprot name not found in table 'uniprot' for symbol (kept)", $symbol);
					addme("uniprot name not found in table 'uniprot' for name (kept)", $name);
					addme("uniprot name not found in table 'uniprot' for acc (kept)", $acc);
				}
				else
				{
					...
					if ($uniprot_hgnc ne $symbol)
					{
						...
						addme("uniprot symbol mismatch for symbol (kept)", $symbol);
						addme("uniprot symbol mismatch for acc (kept)", $acc);
					}
					else
					{
						addme("uniprot symbol match for symbol", $symbol);
						addme("uniprot symbol match for acc", $acc);
					}
					# {
	...
	while (($othersymbol, $alias) = Fetch($query))
	{
		...
		if ($othersymbol eq $alias)
		{
			...
			addme("weird: official symbol is also recorded as an alias for itself for symbol (skipped)", $othersymbol); 
		}
	...
	addme("adding symbol as pseudo-alias for symbol", $symbol);
