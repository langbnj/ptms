...
$mainquery = Query("SELECT alias, GROUP_CONCAT(DISTINCT symbol ORDER BY symbol SEPARATOR '|'), COUNT(DISTINCT symbol) AS c FROM hgnc_aliases GROUP BY alias ORDER BY c DESC;");
startme("Removing ambiguous aliases for all symbols in table 'hgnc_aliases'", 0, Numrows($mainquery));
starttime();
$affected = 0;
while (($alias, $symbols, $symbol_count) = Fetch($mainquery))
{
	...
	if ($symbol_count > 1)
	{
		if (!switch('debug'))
		{
			$query = Query("DELETE FROM hgnc_aliases WHERE alias='".esc($alias)."'");
		}
		else
		{
			$query = Query("SELECT * FROM hgnc_aliases WHERE alias='".esc($alias)."'");
		}
