...
startme("Checking how many 'human' modified proteins from 'unimod' have the proper amino acids in their 'ensembl' seqs");
starttime();
	...
	while (($ensp) = Fetch($enspquery))
	{
		...
		while (($site, $ptm) = Fetch($ptmquery))
		{
			...
			if (Numrows($query) == 0)
			{
				...
				addme("no sequence in ensembl for acc", $acc);
...
showme("no sequence in ensembl for acc", 1);
