...
# Install modules
# sudo apt-get install libmysqlclient-dev # For DBD::mysql's 'mysql_config' test (presumably I did this on the MySQL server, where I have sudo?)
# cpan
# # old
# # install Term::ReadKey Term::ReadLine::Perl Bundle::CPAN CPAN::DistnameInfo Log::Log4perl XML::Parser Bio::Phylo Statistics::Descriptive Statistics::R Text::CSV List::Compare Sort::Key::Natural Carp::Always Math::Round Statistics::Robust::Scale List::Vectorize Statistics::Multtest Statistics::RankCorrelation DBD::mysql

# # Force install packages with permissions test failures at St Jude:
# force install Log::Log4perl
# force install ExtUtils::MakeMaker
# force install IO::Compress::Base
# force install File::Temp
# force install File::Copy::Recursive

# # Install the rest normally, but one by one in order to be able to debug
# install Bundle::CPAN
...
# install Carp::Always
# install Math::Round
# install Statistics::Robust::Scale
# install List::Vectorize
# install Statistics::Multtest
# install Statistics::RankCorrelation
# install DBD::mysql

# Install openssl library (compile it: https://www.openssl.org/source/)
# Install libssh2 library (compile it: https://www.libssh2.org) (ignore errors, they relate to docker not being installed)
# Install libgit2 library (compile it: https://libgit2.org)

# R
# Prereq for phylosim (can't install it directly from CRAN because of a non-renewed licence agreement or something):
# install.packages("https://cran.r-project.org/src/contrib/Archive/compoisson/compoisson_0.3.tar.gz", repos=NULL, type="source")
# Phylosim (only not on CRAN because compoisson went missing):
# install.packages("https://cran.r-project.org/src/contrib/Archive/phylosim/phylosim_3.0.5.tar.gz", repos=NULL, type="source")

# Install all packages I ever used:
# install.packages("phytools")
# install.packages(c("beeswarm", "boot", "changepoint", "coin", "devtools", "dplyr", "egg", "fdrtool", "forcats", "GGally", "ggbeeswarm", "ggplot2", "ggrepel", "glue", "gplots", "gridExtra", "heatmaply", "Hmisc", "igraph", "inflection", "magrittr", "mixtools", "naturalsort", "phylosim", "phytools", "plotROC", "plyr", "poweRlaw", "pROC", "RColorBrewer", "reshape2", "RMariaDB", "RMySQL", "RSQLite", "scales", "stringr", "testthat", "tibble", "tidyr", "tidyverse", "VennDiagram", "viridis", "wesanderson"))
...
# BiocManager::install(c("clusterProfiler", "GenomicFeatures", "GenomicRanges", "GO.db", "org.Hs.eg.db", "ReactomePA", "reactome.db"), lib="~/R/x86_64-pc-linux-gnu-library/4.0")
# # BiocManager::install(lib="~/R/x86_64-pc-linux-gnu-library/4.0")
# # BiocManager::install("clusterProfiler", lib="~/R/x86_64-pc-linux-gnu-library/4.0")
# # BiocManager::install("GenomicFeatures", lib="~/R/x86_64-pc-linux-gnu-library/4.0")
# # BiocManager::install("GenomicRanges", lib="~/R/x86_64-pc-linux-gnu-library/4.0")
# # BiocManager::install("GO.db", lib="~/R/x86_64-pc-linux-gnu-library/4.0")
# # BiocManager::install("org.Hs.eg.db", lib="~/R/x86_64-pc-linux-gnu-library/4.0")
# # BiocManager::install("ReactomePA", lib="~/R/x86_64-pc-linux-gnu-library/4.0")
# # BiocManager::install("reactome.db", lib="~/R/x86_64-pc-linux-gnu-library/4.0")
...
# Bio::Phylo::IO dependencies:
# compiled & installed libgd
# force install GD
# force install Net::HTTP
...
# install XML::LibXML
# install SWF::Builder
# install Bio::Phylo::IO
# # Note: I'm not sure what causes the SIGWINCH bug when resizing the terminal while in cpan. It must be a problematic package: initially it isn't there, then it is after installing a couple of packages. I'm not sure which one. Can use 'cpanm install ...' instead.
...
sub comparaprune
{
	...
	while ($old ne $tree->to_newick)
	{
		...
		# Node labels (node->get_name) will sometimes be blank (''), but it's not because they're unbranched internals. I suppose they're branched internal nodes, then.
		# # Collapse unbranched internal nodes
		# $tree->remove_unbranched_internals;
		...
		foreach my $node (@{$tree->get_root->get_terminals()})
		{
			...
			my $query = Query("SELECT species FROM comparaspecies WHERE species='$species'");
			...
			elsif (Numrows($query) == 0)
			{
				...
				addme("comparaprune: node id not a comparaspecies for species", $species) if (!$silent);
...
sub showbranchlengths
{
	...
	foreach my $node (@{$tree->get_root->get_terminals()})
	{
		...
		# Rounded to 6 digits:
		# print " >> ".$node->get_name." >> ".round($node->calc_path_to_root(), 6)."\n";
	}
}
...
sub characterise
{
	...
	if (scalar(@a) > 0)
	{
		print "NUM ".scalar(@a)."\t"."SUM ".sum(@a)."\t"."MIN ".sprintf("%.2f", min(@a))."\t"."AVG ".sprintf("%.2f", avg(@a))."\t"."MED ".sprintf("%.2f", median(@a))."\t"."MAX ".sprintf("%.2f", max(@a))."\t"."STD ".sprintf("%.2f", stdev(@a))."\n";
	}
	else
	{
...
#
# $flippedseq = strandflip($dnaseq);							# Flips DNA strand (reverse complement)
...
sub translate_mitochondrial
{
	my ($in) = @_;
	my $out = '';

    # Mitochondrial changes:
    # AGA >> * instead of R		# Apparently not, after all - these codons are apparently rare in mitochondria and induce a -1 frameshift leading to termination via UAG: https://science.sciencemag.org/content/327/5963/301 They're both extremely rare codons, only occurring C-terminally in Ensembl. Only sorfs.org considers them valid internal "R" codons as well.
    # AGG >> * instead of R		# Apparently not, after all - these codons are apparently rare in mitochondria and induce a -1 frameshift leading to termination via UAG: https://science.sciencemag.org/content/327/5963/301 They're both extremely rare codons, only occurring C-terminally in Ensembl. Only sorfs.org considers them valid internal "R" codons as well.
    # ATA >> M instead of I
	...
	foreach $triplet ($in =~ /.../g)
	{
		...
		elsif (($triplet eq 'AGA') and (($pos <  length($in)) or (length($in) == 3))) { $out .= 'R'; }	# mitochondria-specific (see above): Only sorfs.org uses this as a non-stop codon in mitochondria. It only occurs terminally in Ensembl.
		elsif (($triplet eq 'AGA') and ($pos == length($in))) { $out .= '*'; }  # mitochondria-specific (see above)
		elsif ($triplet eq 'AGC') { $out .= 'S'; }
		# elsif ($triplet eq 'AGG') { $out .= 'R'; }
		# elsif ($triplet eq 'AGG') { $out .= '*'; }  # mitochondria-specific
		elsif (($triplet eq 'AGG') and (($pos <  length($in)) or (length($in) == 3))) { $out .= 'R'; }	# mitochondria-specific (see above): Only sorfs.org uses this as a non-stop codon in mitochondria. It only occurs terminally in Ensembl.
		elsif (($triplet eq 'AGG') and ($pos == length($in))) { $out .= '*'; }  # mitochondria-specific (see above)
		elsif ($triplet eq 'AGT') { $out .= 'S'; }
		# elsif ($triplet eq 'ATA') { $out .= 'I'; }
...
sub translate_loosely
{
    ...
    # die("Error while translating: Sequence length is not a multiple of three (".length($in)." / 3 = ".sprintf("%.3f", (length($in) / 3))."): '$in'") if (length($in) % 3 != 0);
    # die("Error while translating: Sequence contains characters other than A, C, G, T, N, -: '$in'") if ($in !~ /^[ACGTN\-]+$/);
    # die("Error while translating: Sequence contains characters other than A, C, G, T, N, K, M, R, S, W, Y -: '$in'") if ($in !~ /^[ACGTNKMRSWY\-]+$/);
    # die("Error while translating: Sequence contains characters other than A, C, G, T, N, K, M, R, S, W, Y, D -: '$in'") if ($in !~ /^[ACGTNKMRSWYD\-]+$/);
	if ($in !~ /^[ACGTBNDHKMRSVWY\-]+$/)
	{
		$tmpin = $in;
		$tmpin =~ s/[ACGTBNDHKMRSVWY\-]+//g;
		die("Error while translating: Sequence contains characters other than A, C, G, T, N, B, D, H, K, M, R, S, V, W, Y, -: '$tmpin'\n\n'$in'\n\n") ;
	}
    ...
    foreach $triplet ($in =~ /.../g)
    {
        ...
        # Ambiguity (only handling observed cases, one at a time)
        ...
        elsif ($triplet eq 'GYD') { $out .= 'X'; }
        elsif ($triplet eq 'GYG') { $out .= 'X'; }
        elsif ($triplet eq 'HCA') { $out .= 'X'; }
...
# showmeall(1);							# show all lists, ever! but compactly.	unique()s the lists.
...
sub mynodes			# get number of BLADE farm CPUs currently running my jobs.
{
	...
	elsif ((qtype() eq 'lsf') or (qtype() eq 'node'))
	{
		# LSF (Note: It's actually just the job number here, not number of CPUs)
		$tmp = `bjobs -w -u \$USER 2>/dev/null | awk '{print \$3}' | grep -v 'STAT' | grep -P '(RUN|PEND)' | wc -l`;
		chomp($tmp);
		$n += $tmp;
	}
	else
...
sub interactivenodes			# get number of interactive jobs that I have running
{
	my $n = 0;
	...
	{
		# LSF (Note: It's actually just the job number here, not number of CPUs)
		$tmp = `bjobs -w -u \$USER -q interactive 2>/dev/null | awk '{print \$7}' | grep -v 'JOB_NAME' | wc -l`;
		chomp($tmp);
		$n += $tmp;
	}
	else
...
sub thesenodes			# get number of CPUs currently running jobs from a particular pipeline
{
	my $n = 0;
    ...
    elsif ((qtype() eq 'lsf') or (qtype() eq 'node'))
    {
		...
		$tmp = `bjobs -w -u \$USER 2>/dev/null | awk '{print \$7}' | grep -v 'JOB_NAME' | grep -P '^$locale\_' | wc -l`;
		chomp($tmp);
		$n += $tmp;
    }
	else
...
sub queuednodes
{
	...
	{
		$tmp = `bjobs -w -u \$USER 2>/dev/null | awk '{print \$3}' | grep -v 'STAT' | grep 'PEND' | wc -l`;
		chomp($tmp);
		$n += $tmp;
	}
...
sub qtype
{
	...
	# elsif (contains($hostname, @mainframes_lsf) or ($hostname =~ /^noderome\d+$/) or ($hostname =~ /^nodecn\d+$/))
...
sub waitforjobs			# Wait for jobs from a particular script pipeline directory to finish
{
	if (qtype() ne 'node')
	{
		...
		# $minnodes = 0 + interactivenodes();
		$minnodes = 0;
	}
	else
	{
		# if script is running on its own node:
		# $minnodes = 1 + interactivenodes();
		$minnodes = 1;
	}

	state("Waiting for these jobs to finish (currently ".thesenodes().", expecting $minnodes)...");
...
sub waitforalljobs			# Wait for all of my jobs to finish
{
	...
	state("Waiting for all jobs to finish (currently ".mynodes().", expecting $minnodes)...");
...
sub getnumjobs			# LEGACY == get number of CPUs currently in use. slightly dodgy but works. == LEGACY
{
	my $j = 0;
...
sub ttest
{
	...
	while (<R>)
	{
		...
		if (/^ \$ p\.value\s+: num (NaN|[\d\.e\-\+]+)$/)
		{
			$p = $1;

			# Handle NaN (if e.g. there are too few values to compare)
			if ($p eq 'NaN')
			{
				$p = 1;
			}
...
sub jaccard
{
    # Return Jaccard index between two sets (intersection divided by union)
	# Note: Returns 0 if the size of the union is 0 (i.e. for two empty sets)
    my ($a, $b) = @_;
	
    my @union = union($a, $b);
    my @intersection = intersection($a, $b);
...
sub positions
{
	# get positions of a substring in a string, returned as an array. Starts counting at 1.
	# case sensitive
	my ($s, $l) = @_;
...
sub non_overlapping
{
	# Get non-overlapping regions within an array of coordinates (1-3, 2-5, 1-10)
	# This is written with protein domains in mind, where one larger family definition can overlap multiple smaller motifs.
	# The algorithm starts with the smallest region the farthest on the left, and then adds non-overlapping regions by ascending start position and size and ultimately returns them.
	# Regions that overlap others are ignored.
	...
	foreach my $region (@regions)
	{
		...
		$region =~ /^(\d+)-(\d+)$/ or die("Error: Region format should be e.g. '1-2' for '$region'");
		my $start = $1;
		my $stop = $2;
		...
		# Get width
		my $width = $stop - $start + 1;
		$width{$region} = $width;
		push(@widths, $width);
	}
	
	# Set pseudo-sequence string to the right length so it can be manipulated with substr
	my $s = '.' x $max;
	# foreach my $region (sort { $width{$a} <=> $width{$b} } nsort(@regions))
	# First sort by width...
	foreach my $width (unique(@widths))
	{
		# ...then by start position
		foreach my $region (nsort(keys(%width)))
		{
			next if $width{$region} != $width;
			...
			my $width = $width{$region};
			
			# If this new region doesn't overlap with any existing one, add it to the pseudo-sequence
			if (substr($s, $start, $width) !~ /\*/)
			{
				substr($s, $start, $width) = '*' x $width;
				# print "\n >> Width $width: $region\n   >> $s\n\n";
			}
		}
	}
	
	# state("Non-overlapping now:");
	# Re-extract non-overlapping regions from pseudo-sequence string
	my @res = ();
	while ($s =~ /(\*+)/g)
	{
		my $stop = pos($s) - 1;
...
sub ymd
{
	...
	# Separator can be specified, e.g. ymd('_') (default: '-')
	if (scalar(@_) == 1)
	{
		($sep) = @_ ;
	}
	
	strftime('%Y'.$sep.'%m'.$sep.'%d', localtime);
}
